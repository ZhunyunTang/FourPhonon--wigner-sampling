...
!zhunyun tang  ----Output thermal conductivity at each frequency 10(off-diagonal term) 20(diagonal term) 
open(unit=10, file='ThConductivityCohMode_fre', status='replace', action='write')
open(unit=20, file='ThConductivityMode_fre', status='replace', action='write')
    do jj=1,Nbands
       do ii=2,nptk
          do dir1=1,3
             do dir2=1,3
 ...
 ! zhunyun
    ThConductivityCohMode(ii,jj,kk,:,:)=1e21*hbar**2*ThConductivityCohMode(ii,jj,kk,:,:)/(kB*T*T*V*nptk)
              call symmetrize_tensor(ThConductivityCohMode(ii,jj,kk,:,:))
